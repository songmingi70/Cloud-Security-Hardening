...
### 🔍 실무형 트러블슈팅 사례
1. **문제**: Nginx 설정 후 403 Forbidden 에러 발생
2. **원인 분석**: 웹 루트 디렉토리(/var/www/html) 내 인덱스 파일 부재 및 권한 설정 미흡.
3. **해결**: `index.html` 생성 및 `chown www-data`, `chmod 644`를 통한 권한 교정.
4. **문제**: 한글 깨짐 현상 발생
5. **해결**: HTML 메타 태그(`utf-8`) 추가로 인코딩 교정.
---
